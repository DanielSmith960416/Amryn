/**
 * Sending mail, without the React-bundler guard.
 *
 * This is smtp.ts with `import 'server-only'` taken off the top, and nothing
 * else. The split exists because the worker is a plain Node process that must
 * be able to email a morning brief, and the guard — which is a marker for the
 * React bundler, not a runtime protection — makes the worker build refuse the
 * file by design.
 *
 * The same split was made for the AI provider's error types in #70, for the
 * same reason and with the same shape: the guarded module re-exports this one,
 * so every caller that should be guarded still is, and nothing is duplicated.
 *
 * The credential still never reaches a browser. What stops that is that this
 * module is imported only by server code and by the worker, and that the
 * bundle scan in CI fails the build if an SMTP value appears in client output.
 */
