...
export const SETTINGS: readonly Setting[] = [
  ...
  { name: 'AMRYN_SERVICE', stage: 'platform', required: false, secret: false, purpose: 'Which program is running — the worker sets it on itself at startup, so a recorded failure says whether to look at the worker or the web service. Never set by hand.', withoutIt: 'Errors are recorded as the web service, which is right for everything except the worker.' },
