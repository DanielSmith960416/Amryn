...
describe('workerHealth', () => {
  it('reports a worker that beat a moment ago as running', () => {
    const health = workerHealth({ beat: beat(3) }, NOW);
  ...
  it('does not call an idle worker unhealthy', () => {
    ...
    const health = workerHealth({ beat: beat(20, { inFlight: 0 }) }, NOW);
    expect(health.state).toBe('running');
  });
  ...
  it('tolerates a deploy replacing the worker', () => {
    ...
    expect(workerHealth({ beat: beat(STALE_AFTER_SECONDS + 30) }, NOW).state).toBe('stale');
  });

  it('calls it gone once no restart could explain it', () => {
    const health = workerHealth({ beat: beat(MISSING_AFTER_SECONDS + 60) }, NOW);
  ...
  it('names what stops when the worker is gone, rather than saying "unhealthy"', () => {
    const health = workerHealth({ beat: beat(3600) }, NOW);
  ...
  it('distinguishes never-started from died-an-hour-ago', () => {
    ...
    const never = workerHealth({ beat: null }, NOW);
  ...
  it('is exact at the boundaries rather than approximately right', () => {
    expect(workerHealth({ beat: beat(STALE_AFTER_SECONDS) }, NOW).state).toBe('running');
    expect(workerHealth({ beat: beat(STALE_AFTER_SECONDS + 1) }, NOW).state).toBe('stale');
    expect(workerHealth({ beat: beat(MISSING_AFTER_SECONDS) }, NOW).state).toBe('stale');
    expect(workerHealth({ beat: beat(MISSING_AFTER_SECONDS + 1) }, NOW).state).toBe('gone');
  });

  it('never reports a negative age when a clock disagrees', () => {
    // The worker's clock and the reader's are different machines.
    const health = workerHealth({ beat: beat(-30) }, NOW);
    expect(health.state).toBe('running');
    if (health.state !== 'running') return;
...
describe('a reading that failed', () => {
  /*
   * The distinction this exists for. A page that cannot reach the database
   * knows nothing about the worker, and saying "no worker has ever reported"
   * is a false alarm about a process that may be running perfectly.
   */
  it('is never reported as a worker that has not run', () => {
    const health = workerHealth({ beat: null, problem: 'connection refused' }, NOW);
    expect(health.state).toBe('unreachable');
    expect(health.state).not.toBe('never');
  });

  it('says what stopped it, so the reader can tell whose fault it is', () => {
    const health = workerHealth({ beat: null, problem: 'password authentication failed' }, NOW);
    expect(health.state === 'unreachable' && health.detail).toContain(
      'password authentication failed',
    );
    expect(health.state === 'unreachable' && health.detail).toContain('Could not reach the database');
  });

  /*
   * Ranked above every other state, including a beat old enough to read as an
   * outage: a stale beat read over a broken connection is not evidence.
   */
  it('outranks a beat that came with it, because the reading is not trustworthy', () => {
    const health = workerHealth(
      { beat: beat(MISSING_AFTER_SECONDS + 600), problem: 'timeout' },
      NOW,
    );
    expect(health.state).toBe('unreachable');
  });

  it('leaves a clean reading with no beat reading as never, as before', () => {
    expect(workerHealth({ beat: null }, NOW).state).toBe('never');
  });
});
